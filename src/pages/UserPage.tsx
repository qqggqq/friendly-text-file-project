...
import { Calendar, Plus, Filter, Check, X, Users, Eye, LogOut, DollarSign } from 'lucide-react';
...
const UserPage = () => {
  ...
  const canManageBudgets = isAdmin || isTreasurer;
  ...
  const getBudgetStatusBadge = (status: string) => {
    switch (status) {
      case 'approved':
        return <Badge className="bg-green-100 text-green-800">Budget Approved</Badge>;
      case 'pending':
        return <Badge className="bg-yellow-100 text-yellow-800">Budget Pending</Badge>;
      case 'denied':
        return <Badge className="bg-red-100 text-red-800">Budget Denied</Badge>;
      default:
  ...
  if (isAdmin || isTreasurer) {
    const roleTitle = isAdmin ? "Admin Dashboard" : "Treasurer Dashboard";
    const roleDescription = isAdmin ? 
      "AURAK Event Management Platform - Administrative Panel" : 
      "AURAK Event Management Platform - Budget Management";
              ...
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-3xl font-bold flex items-center">
                    {isAdmin ? <Users className="w-8 h-8 mr-3" /> : <DollarSign className="w-8 h-8 mr-3" />}
                    {roleTitle}
                  </CardTitle>
                  ...
                  <TableBody>
                    {filteredEvents.map((event) => (
                      <TableRow key={event.id} className="hover:bg-red-50">
                        ...
                        <TableCell>{getBudgetStatusBadge(event.budgetStatus)}</TableCell>
                        <TableCell>
